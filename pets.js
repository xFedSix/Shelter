...
const petCount = largePetData.length;

const pageBtnNext = document.querySelector(".pets_page-btn--next");
const pageBtnPrev = document.querySelector(".pets_page-btn--previous");
const pageBtnLast = document.querySelector(".pets_page-btn--last");
...
pageBtnFirst.addEventListener("click", () => {
  pageNum = 1;
  petsPage.textContent = pageNum;
  deletePets();
  addPets();
...
const addPets = () => {
  ...
  if (pageSize === "mobile") {
    ...
    numIndex.forEach((element) => {
      let template = "";
      const card = document.createElement("div");
      template += `<img src="${element.img}" alt="img" />`;
      ...
      petCards.insertAdjacentElement("afterbegin", card);
    });
  }
  if (pageSize === "tablet") {
    let numIndex = mixedArr.slice(0, 6);
    numIndex.forEach((element) => {
      let template = "";
      const card = document.createElement("div");
      template += `<img src="${element.img}" alt="img" />`;
  ...
  if (pageSize === "decktop") {
    let numIndex = mixedArr.slice(0, 8);
    numIndex.forEach((element) => {
      let template = "";
      const card = document.createElement("div");
      template += `<img src="${element.img}" alt="img" />`;
...
const deletePets = () => {
  while (petCards.firstChild) {
    petCards.removeChild(petCards.firstChild);
  }
};
addPets();
...
petCards.addEventListener("click", addModal);
